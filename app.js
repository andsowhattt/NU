...
class Github {
	...
	async getRepos(userName) {
		const response = await fetch(`${GITHUB_API}users/${userName}/repos?per_page=${this.reposCount}&sort=${this.reposSort}&client_id=${this.clientId}&client_secret=${this.clientSecret}`);
		if (response.status === 404) {
			ui.showAlert('User not found', 'alert alert-danger');
		} else {
